/**
 * ap.fullPageSwipeLoop from fullPage 2.5.6
 * https://github.com/AcePeak/fullPage.js
 * MIT licensed
 *
 * Copyright (C) 2015 lilifeng.net - A project by Ace Li
 */
		...
		*/
		...
		$('.fp-section').each(function(index){
			...
			$(window).on('load', function() {
				...
				$('.fp-slidesContainer').each(function(index){
					var slides = $(this).find('.fp-slide');
					if(slides.length > 0)
					{
						silentLandscapeScroll(slides.eq(1));
					}
				});
			});
		});
		...
		*/
		...
		function moveSlide(direction){
			...
			if(options.continuousHorizontal)
			{
				...
				if(cIndex >= allSlides.length - 1 && direction === 'next')
				{
					var realSlide = currentSlide.siblings(':eq(1)');
					silentLandscapeScroll(realSlide);
					currentSlide = realSlide;
				}
				else if(cIndex <= 0 && direction === 'prev')
				{
					var realSlide = currentSlide.siblings(':eq(-2)');
					silentLandscapeScroll(realSlide);
					currentSlide = realSlide;
				}
			}
		...
		*/
		function landscapeScroll(slides, destiny){
			...
			if(options.continuousHorizontal)
			{
				...
				showingIndex = showingIndex % (secSlides.length - 2);
			}
			
			slidesNav.find('li').eq(showingIndex).find('a').addClass('active');
		}
